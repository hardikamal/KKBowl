...
- (void)storeAccessToken
{	
     HKDLog(@"%@,%@,%lf,%@",self.accessToken,self.name,self.expires_Time,self.sharerId);
...
//    self.openkey = [SHK getAuthValueForKey:kQQ_openkey forSharer:sharerId];
    ...
    HKDLog(@"%@,%@,%lf,%@",self.accessToken,self.name,self.expires_Time,self.openid);
...
#warning 无网络状态下，授权页面会授权失败，此时，是否应该有所提示呢？
    if (!success && error) {
        HKDLog(@"%@",[error localizedDescription]);
...
//    [mutableParams setObject:@"all" forKey:@"scope"];
    ...
    [op addCompletionHandler:^(MKNetworkOperation *completedOperation)
     {
         HKDLog(@"%@",[completedOperation responseString]);
